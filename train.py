...
# Train test split, remove target variable and ID
...
def model_metrics(actual, pred, exp_id, run_id):
    ...
    # Save plot
    Path.mkdir(f"mlruns/{exp_id}/{run_id}/plots", exist_ok=True)
    plt.savefig(f"mlruns/{exp_id}/{run_id}/plots/ROC_curve.png")
...
def mlflow_logs(model, X, y, name):
    ...
    with mlflow.start_run(run_name=name) as run:
        ...
        (accuracy, f1, auc) = model_metrics(y, pred, exp_id, run_id)
        ...
        mlflow.log_artifact(f"mlruns/{exp_id}/{run_id}/plots/ROC_curve.png")
